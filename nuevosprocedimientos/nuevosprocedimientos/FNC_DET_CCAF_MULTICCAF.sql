CREATE OR REPLACE FUNCTION X0DESPFA.FNC_DET_CCAF_MULTICCAF(
    p_anio            VARCHAR2,
    p_mes             VARCHAR2,
    p_con_rut         NUMBER,
    p_con_correl      NUMBER,
...
FROM REC_EMPRESA E
JOIN REC_TOTALCCAF TC ON E.REC_PERIODO = TC.REC_PERIODO AND E.CON_RUT = TC.CON_RUT
                     AND E.CON_CORREL = TC.CON_CORREL AND E.RPR_PROCESO = TC.RPR_PROCESO
                     AND E.NRO_COMPROBANTE = TC.NRO_COMPROBANTE
...
JOIN VIS_REC_CAJAS C ON
  ((TO_CHAR(p_con_correl) BETWEEN 600 AND 699 AND S.SUC_CCAF_ADH = C.ENT_CODIFICACION)
   OR (TO_CHAR(p_con_correl) NOT BETWEEN 600 AND 699 AND TC.ENT_RUT = C.ENT_RUT))
