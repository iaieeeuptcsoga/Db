CREATE procedure sp_certCot_trab @fec_ini datetime, @fec_ter datetime, @emp_rut numeric(9),
...
declare @vir_tra table(
	...
	tra_ins_apv numeric(2),
        ...
        emp_multi_ccaf numeric(1),
        emp_multi_mutual numeric(1)
)
	  ...
	  -- VOS agrego  trabajador.tra_rem_imp_afp,
		...
		select 
			...
			trabajador.tra_ins_apv,
                        ...
                        ISNULL(empresa.emp_multi_ccaf,0) AS emp_multi_ccaf,
                        ISNULL(empresa.emp_multi_mutual,0) AS emp_multi_mutual
		...
		where 	empresa.rec_periodo     between @fec_ini and @fec_ter
			...
			and empresa.rpr_proceso     in (1,2,3,4)
	...
	begin
		...
		select 
			...
			pago.pag_fec_timbre as pag_fec_pag,
                        emp_ccaf_adh =
        		CASE 
           		   WHEN (empresa.emp_multi_ccaf is null) or (empresa.emp_multi_ccaf=0)
                		THEN empresa.emp_ccaf_adh  
           		   WHEN (empresa.emp_multi_ccaf=1) 
                		THEN trabajador.tra_ccaf_adh   
           		   ELSE 0
		        END,	
			emp_mut_adh =
                        CASE 
           		   WHEN (empresa.emp_multi_mutual is null) or (empresa.emp_multi_mutual=0)
                		THEN empresa.emp_mut_adh  
           		   WHEN (empresa.emp_multi_mutual=1) 
                		THEN trabajador.tra_mutual_adh   
           		   ELSE 0
		        END,
			empresa.emp_tasa_cot_mut,
			empresa.emp_cot_adic_mut,
...
and vt.rpr_proceso in (1,2)
and vt.tra_rut = @rut_tra
and tra_inp.trainp_cot_acc_trab > 0
order by vt.rec_periodo, vt.suc_cod, vt.usu_codigo
...
select 
	...
	ent_nom = 
        CASE 
           WHEN (isapres.ent_rut = 96504160) and ((year(vt.rec_periodo) = 2014 and month(vt.rec_periodo) < 7) 
                or (year(vt.rec_periodo) < 2014)) 
		THEN 'FERROSALUD S.A.'
...
select  distinct
	...
	END,
        cotizacion = 
	CASE 
          WHEN ((year(vt.rec_periodo) = 2013 and month(vt.rec_periodo) >=11 and vt.emp_multi_mutual = 0)
                or (year(vt.rec_periodo) >= 2014) and vt.emp_multi_mutual = 0)
                THEN (vt.rem_impo_mutual * (tasa_cot_mut + tasa_adic_mut)) / 100   
          WHEN ((year(vt.rec_periodo) = 2013 and month(vt.rec_periodo) >=11 and vt.emp_multi_mutual = 1)
                or (year(vt.rec_periodo) >= 2014) and vt.emp_multi_mutual = 1)
                THEN (vt.rem_impo_mutual * (TOTAL_MUTUAL.TMUT_Tasa_cot_mut + TOTAL_MUTUAL.TMUT_Cot_adic_mut)) / 100   
          WHEN (vt.rpr_proceso = 1 
                AND vt.prevision = 0  
                AND year(vt.rec_periodo) >= 2010)
